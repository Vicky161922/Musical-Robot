...
int count=1,data=0,played=-1,prsnt2=13,nxt2=0,obstacle=0;
...
SIGNAL(USART0_RX_vect) 		// ISR for receive complete interrupt
{
	...
	else if((data>200) && (data<=250))
	{
		...
		cost[prsnt2-1][nxt2-1]=9999;
		cost[nxt2-1][prsnt2-1]=9999;
		obstacle=1;
	}
/*	else if((data>150) && (data<=198))
	{
		prsnt2=(data-150);
	}*/	
}
...
{	
	...
	float l,c,r;
	int p=0,i,j,k,threshold=15,start,dest,steps,boot=0,sharp,obstacle_range=0;
	int prev=0,prsnt=1,nxt=0,len1=0,len2=0,z=0,temp_dest=0,temp_len1=0,temp_len2=0;// denotes the position of nodesi.e. previous, present , next
	int filtered_nodes[33];
	int nodes[33]={24,14,19,43,38,34,8,3,36,25,29,32,37,29,44,1,13,37};//PLEASE CHANGE THE POSITION OF THE PRSNT TO 1
		count=0;
...
horizontal();
		...
		//	prev=prsnt;
			...
			for(int f=0;f<30;f++)
			{
			filtered_nodes[f]=0;			//clear filter node first
			}
	
			z=0;	
	
	
for(int cc=played+1;cc<=33;cc++)		//filtered nodes
{
	 temp_dest=nodes[cc];
	dijkstra(prsnt,temp_dest);
	steps=1;
...
dest=filtered_node[0];
 ...
 if(((prsnt==48) && (nxt==37))||((prsnt==37) && (nxt==48))||((prsnt==40)&&(nxt==41))||((prsnt==41)&&(prsnt==40))||((prsnt==44)&&(nxt==45))||((prsnt==45)&&(nxt==44)))
{	
	obstacle_range=40;
}
else{
 ...
 if ((sharp<=158)&&(sharp>=obstacle_range))
 {
	...
	UDR0=nxt+200;
	_
	while((data<100) && (data>150) ||(data<200) &&(data>250))
	{
...
dest=filtered_node[0];
	...
	prsnt=path[0];
	nxt=path[1];
	k=1;			//pending-> double length path condition, angle turn followed by backward_mm ,count-1
	 p=0;
